...
### A. git clone (개발자 추천 — 자동 업데이트 포함)

```bash
git clone https://github.com/ggombee/forge-glow.git ~/Desktop/workspace/forge-glow
bash ~/Desktop/workspace/forge-glow/install.sh
```

자동 갱신이 launchd(macOS) 또는 cron(Linux)으로 **1시간 간격** 등록됩니다 (`tools/self-update.sh`).
...
forge-glow install          # statusLine 등록
forge-glow update           # 수동 업데이트
```

### C. Python 대시보드만

```bash
pip install forge-glow-stats
forge-glow-stats            # rich 대시보드 자동 갱신
forge-glow-stats --once     # 1회 렌더
forge-glow-stats --json     # JSON 출력
forge-glow-stats --org      # Admin Analytics API 포함 (ANTHROPIC_ADMIN_API_KEY 필요)
```
